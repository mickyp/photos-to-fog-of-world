...
只要是使用預設輸出方式，不論是 skill、CLI 還是 GUI，產出的檔名都會一致地帶上到秒的時間戳記，方便分辨每一次匯出的版本。

## 資料夾怎麼選

一般情況下，請直接選你要匯出的「上層資料夾」，不用一路點到最深的最後一層資料夾。
...
如果你選的是 `2024`，工具會自動一路往下掃描，必要時分批處理子資料夾，最後再把各處整理出的軌跡合併回 `2024` 這一層。
...
如果你選的是 `Day 1`，工具就只會匯出這一小層，不會知道你其實想要的是整個 `2024` 的合併結果。

## 轉換規則

- 從你選的資料夾開始一路遞迴往下掃描照片
- 若輸入是像 `2015` 這種年度資料夾，會先處理第一層子資料夾，再把結果合併回上層年度資料夾
- 若某個第一層子資料夾底下的下一層照片量總和超過 `1000`，會先切得更細分批處理，再往上合併
...
- `Photo folder`：要掃描的上層照片資料夾。通常選旅行、月份或年份那一層就可以，程式會自動繼續往下掃描。
- `Output GPX (optional)`：GPX 要存到哪裡。若留白，程式會自動在你選的上層照片資料夾裡建立一個帶時間的 GPX 檔。
...
- 除非你只想匯出最底下那一小層，否則不要特地點到最深的最終資料夾。
